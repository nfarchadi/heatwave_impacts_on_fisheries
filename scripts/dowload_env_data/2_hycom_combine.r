...
bathy <- here("data","env_data","NWA","bathy","NWA_bathy.nc")
rugosity <- here("data","env_data","NWA","bathy","NWA_bathysd.nc")
...
for (i in 1:length(udates)) {
  ## water u
  water_u <- list.files(path = here("data","env_data","NWA", "hycom"),
                        full.names=TRUE) %>% 
    grep(pattern = paste0("u_",udates[i]),value = TRUE) %>% raster()
  
  ## water v
  water_v <- list.files(path = here("data","env_data","NWA", "hycom"),
                        full.names=TRUE) %>% 
    grep(pattern = paste0("v_",udates[i]),value = TRUE) %>% raster()
  
  ## ild
  ild <- list.files(path = here("data","env_data","NWA", "hycom"),
                    full.names=TRUE) %>% 
    grep(pattern = paste0("ild5_",udates[i]),value = TRUE) %>% raster()
  
  ## n2
  n2 <- list.files(path = here("data","env_data","NWA", "hycom"),
                   full.names=TRUE) %>% 
    grep(pattern = paste0("n2_",udates[i]),value = TRUE) %>% raster()
  
  ## sst_sd
  sst_sd <- list.files(path = here("data","env_data","NWA", "hycom"),
                       full.names=TRUE) %>% 
    grep(pattern = paste0("SST_SD_",udates[i]),value = TRUE) %>% raster()
  
  ## sst
  sst <- list.files(path = here("data","env_data","NWA", "hycom"),
                        full.names=TRUE) %>% 
    grep(pattern = paste0("u_",udates[i]),value = TRUE) %>% raster()
  
  ## ssh_sd
  ssh_sd <- list.files(path = here("data","env_data","NWA", "hycom"),
                       full.names=TRUE) %>% 
    grep(pattern = paste0("SSH_SD_",udates[i]),value = TRUE) %>% raster()
  
  ## ssh
  ssh <- list.files(path = here("data","env_data","NWA", "hycom"),
                       full.names=TRUE) %>% 
    grep(pattern = paste0("ssh_",udates[i]),value = TRUE) %>% raster()
  
  ## eke
  eke <- list.files(path = here("data","env_data","NWA", "hycom"),
                       full.names=TRUE) %>% 
    grep(pattern = paste0("EKE_",udates[i]),value = TRUE) %>% raster()
  
  ## salinity
  salinity <- list.files(path = here("data","env_data","NWA", "hycom"),
                       full.names=TRUE) %>% 
  ...
  raster::writeRaster(combine, filename = here("data","env_data","NWA", "hycom", 
                                               "hycom_combine", fname), 
                      overwrite=T)
