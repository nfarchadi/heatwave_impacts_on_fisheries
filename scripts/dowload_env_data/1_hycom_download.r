...
envdir<-here("data","NWA","env_data", "hycom")
...
for (i in 1:length(udates)){
  ...
  myURL <- try(HMMoce:::get.hycom(limits=c(-100, -30, 10, 50), time=udates[i],
...
sstList <- list.files(path = here("data","NWA","env_data", "hycom")) %>% 
...
for (i in 1:length(sstList)) {
  sst <- raster::raster(here("data","NWA","env_data", "hycom", sstList[i]))
  ...
  raster::writeRaster(sst_sd, filename = here("data","NWA","env_data", "hycom", fname), 
                      overwrite=T)
}
...
sshList <- list.files(path = here("data","NWA","env_data","hycom")) %>% 
  grep(pattern = "ssh",value = TRUE)

## calculate SSH_SD
for (i in 1:length(sshList)){
  ssh <- here("data","NWA","env_data", "hycom", sshList[i]) %>% 
    raster::raster()
  ssh_sd <- raster::focal(ssh, w = matrix(1, nrow = 3, ncol = 3),
                          fun = function(x){stats::sd(x, na.rm = T)})
  fname <- paste0('hycom_SSH_SD', 
                 substr(sshList[i], 10, nchar(sshList[i])))
  raster::writeRaster(ssh_sd, filename = here("data","NWA","env_data", "hycom", fname), 
                      overwrite=T)
}
...
water_u_files<-list.files(path = here("data","NWA","env_data","hycom")) %>% 
  grep(pattern = "u", value = TRUE)

water_v_files<-list.files(path = here("data","NWA","env_data","hycom")) %>% 
...
for (i in 1:length(water_u_files)){
  for (j in i:length(water_v_files)){
    u <- raster::raster(here("data","NWA","env_data", "hycom", water_u_files[i]))
    v <- raster::raster(here("data","NWA","env_data", "hycom", water_v_files[j]))
    u_v <- raster::stack(u, v)
    eke <- calc(u_v, calculate_eke)
    fname <- paste0('hycom_EKE', 
                   substr(water_u_files[i], 8, nchar(water_u_files[i])))
    raster::writeRaster(eke, filename = here("data","NWA","env_data", "hycom", fname), 
                        overwrite=T)
  }
}
...
water_temp_files<-list.files(path = here("data","NWA","env_data","hycom")) %>% 
  grep(pattern = "temp", value = TRUE)


salinity_files<-list.files(path = here("data","NWA","env_data","hycom")) %>% 
  grep(pattern = "sal", value = TRUE)

# get depth var indices
nc <- here("data","NWA","env_data", "hycom",'hycom_temp_2012-01-01.nc') %>% 
  RNetCDF::open.nc()
...
for (i in 1:length(water_temp_files)){
  ...
  wtemp <- here("data","NWA","env_data", "hycom", water_temp_files[i]) %>% 
    raster::brick(varname = 'water_temp') #* scale + offset
  #calcualte ild
  ild.5 <- calc(wtemp, calculate.ild)
  writeRaster(ild.5, paste0(here("data","NWA","env_data", "hycom", "hycom_ild5"), 
...
system.time({
  foreach(i = 1:nrow(sst_sal_list), .packages = c('raster','here')) %dopar% {
    ...
    wtemp <- raster::brick(here("data","NWA","env_data", "hycom",  wtemp_date),
                           varname = 'water_temp')
    sal_date <-sst_sal_list[i,c("salinity_files")]
    sal <- raster::brick(here("data","NWA","env_data", "hycom",  sal_date), 
                         varname = 'salinity')
    sst_sal_brick <- raster::stack(wtemp[[1:23]], sal[[1:23]])
    n2 <- calc(calc(sst_sal_brick, calculate_n2), fun = function(x) {mean(x, na.rm=T)})
    writeRaster(n2, paste0(here("data","NWA","env_data", "hycom", "hycom_n2"), 
