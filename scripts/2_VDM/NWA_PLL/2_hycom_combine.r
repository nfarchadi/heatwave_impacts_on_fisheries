...
rugosity <- here("data","static","NWA_PLL","NWA_bathysd.nc")
...
for (i in 1:length(udates)) {
  ## water u
  water_u <- list.files(path = here("data","hycom","NWA_PLL"),
                        full.names=TRUE) %>% 
    grep(pattern = paste0("u_",udates[i]),value = TRUE) %>% raster()
  
  ## water v
  water_v <- list.files(path = here("data","hycom","NWA_PLL"),
                        full.names=TRUE) %>% 
    grep(pattern = paste0("v_",udates[i]),value = TRUE) %>% raster()
  
  ## ild
  ild <- list.files(path = here("data","hycom","NWA_PLL"),
                    full.names=TRUE) %>% 
    grep(pattern = paste0("ild5_",udates[i]),value = TRUE) %>% raster()
  
  ## n2
  n2 <- list.files(path = here("data","hycom","NWA_PLL"),
                   full.names=TRUE) %>% 
    grep(pattern = paste0("n2_",udates[i]),value = TRUE) %>% raster()
  
  ## sst_sd
  sst_sd <- list.files(path = here("data","hycom","NWA_PLL"),
                       full.names=TRUE) %>% 
  ...
  ## ssh
  ssh <- list.files(path = here("data","hycom","NWA_PLL"),
                       full.names=TRUE) %>% 
    grep(pattern = paste0("ssh_",udates[i]),value = TRUE) %>% raster()
  
  ## eke
  eke <- list.files(path = here("data","hycom","NWA_PLL"),
                       full.names=TRUE) %>% 
    grep(pattern = paste0("EKE_",udates[i]),value = TRUE) %>% raster()
  
  ## salinity
  salinity <- list.files(path = here("data","hycom","NWA_PLL"),
                       full.names=TRUE) %>% 
