...
envdir<-here("data","hycom","NWA_PLL")
...
sstList <- list.files(path = here("data","hycom","NWA_PLL")) %>% 
...
for (i in 1:length(sstList)) {
  sst <- raster::raster(here("data","hycom","NWA_PLL", sstList[i]))
  ...
  raster::writeRaster(sst_sd, filename = here("data","hycom","NWA_PLL", fname), 
                      overwrite=T)
}
...
water_u_files<-list.files(path = here("data","hycom","NWA_PLL")) %>% 
  grep(pattern = "u", value = TRUE)

water_v_files<-list.files(path = here("data","hycom","NWA_PLL")) %>% 
...
for (i in 1:length(water_u_files)){
  for (j in i:length(water_v_files)){
    u <- raster::raster(here("data","hycom","NWA_PLL", water_u_files[i]))
    v <- raster::raster(here("data","hycom","NWA_PLL", water_v_files[j]))
    u_v <- raster::stack(u, v)
    eke <- calc(u_v, calculate_eke)
    fname <- paste0('hycom_EKE', 
                   substr(water_u_files[i], 8, nchar(water_u_files[i])))
    raster::writeRaster(eke, filename = here("data","hycom","NWA_PLL", fname), 
                        overwrite=T)
  }
}
...
water_temp_files<-list.files(path = here("data","hycom","NWA_PLL")) %>% 
  grep(pattern = "temp", value = TRUE)


salinity_files<-list.files(path = here("data","hycom","NWA_PLL")) %>% 
  grep(pattern = "sal", value = TRUE)

# get depth var indices
nc <- here("data","hycom","NWA_PLL",'hycom_temp_2012-01-01.nc') %>% 
  RNetCDF::open.nc()
...
for (i in 1:length(water_temp_files)){
  ...
  wtemp <- here("data","hycom","NWA_PLL", water_temp_files[i]) %>% 
    raster::brick(varname = 'water_temp') #* scale + offset
  #calcualte ild
  ild.5 <- calc(wtemp, calculate.ild)
  writeRaster(ild.5, paste0(here("data","hycom","NWA_PLL", "hycom_ild5"), 
...
system.time({
  foreach(i = 1:nrow(sst_sal_list), .packages = c('raster','here')) %dopar% {
    ...
    wtemp <- raster::brick(here("data","hycom","NWA_PLL",  wtemp_date),
                           varname = 'water_temp')
    sal_date <-sst_sal_list[i,c("salinity_files")]
    sal <- raster::brick(here("data","hycom","NWA_PLL",  sal_date), 
                         varname = 'salinity')
    sst_sal_brick <- raster::stack(wtemp[[1:23]], sal[[1:23]])
    n2 <- calc(calc(sst_sal_brick, calculate_n2), fun = function(x) {mean(x, na.rm=T)})
    writeRaster(n2, paste0(here("data","hycom","NWA_PLL", "hycom_n2"), 
