...
#VDM stack
VDM_stack<-here("data","Monthly_Spatial_Predictions","NEP_TROLL","NEP_TROLL_monthly_predictions_2012to2020.nc") %>% stack()

tt<-seq(as.Date("2012-01-01"),as.Date("2020-12-01"),by="month") %>% 
  subset(format.Date(., "%m") %in% c("05", "06","07","08","09","10", "11")) %>% 
  as.yearmon()

VDM_stack<-setZ(VDM_stack,tt)

#VDM anomaly stack
VDM_anom_stack<-here("data","Monthly_Spatial_Predictions","NEP_TROLL","NEP_TROLL_monthly_anom_predictions_2012to2020.nc") %>% stack()
...
threshold_finder<-function(VDM_path){
  ...
  for(i in 1:length(VDMdaily_ras)){
    ...
    daily_thresh<-data.frame(quantile75=quant75,
                             date=as.Date(substr(VDMdaily_ras[i], 46,55)),format = "%Y-%m-%d")
...
VDMdaily<-"E:/VDM_results/NEP_TROLL/spatial_predictions"
...
NEP_TROLL_landscapemetrics_monthly<-mgmt_zone_landscapemetrics(VDM_stack,
                                                               NEP_TROLL_zones, thresh)

NEP_TROLL_landscapemetrics_monthly<-bind_rows(NEP_TROLL_landscapemetrics_monthly) %>% dplyr::select(1,2,3,39:51) %>%
...
saveRDS(NEP_TROLL_landscapemetrics_monthly, here("data","Mgmt_zone", "NEP_TROLL","NEP_TROLL_landscapemetrics_monthly.rds"))
...
NEP_TROLL_landscapemetrics_monthly_anomaly<-mgmt_zone_landscapemetrics(VDM_anom_stack,NEP_TROLL_zones,thresh1)

#SDMTools units are in m^2. I want to convert it to km^2
#Need to divide by 1000000 (at least for the area metrics)
NEP_TROLL_landscapemetrics_monthly_anomaly<-bind_rows(NEP_TROLL_landscapemetrics_monthly_anomaly) %>%
...
saveRDS(NEP_TROLL_landscapemetrics_monthly_anomaly, here("data","Mgmt_zone","NEP_TROLL","NEP_TROLL_landscapemetrics_monthly_anomaly.rds"))
