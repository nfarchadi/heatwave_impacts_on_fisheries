...
template<-raster("E:/HYCOM_global/water_temp/hycom_temp_2012-01-01.nc") ## making a SST raster the template

#but its world so I need to change the extent
e<-extent(-180,-109,18,61)
template<-raster::crop(template,e)

#creating a spatial object
US_ALB_ports_shp<-US_ALB_ports
coordinates(US_ALB_ports_shp)<-~lon+lat
#setting the coordinate the same as US_ALB_ports_shp
proj4string(US_ALB_ports_shp) <- CRS(proj4string(template))
dist_port<-distanceFromPoints(template,US_ALB_ports_shp)
dist_port_masked<-mask(dist_port,template)
dist_port_masked<-dist_port_masked*0.001 #converting to km

dist_port_masked<-resample(dist_port_masked, template)
