...
NEP_TROLL_zonesOLD<-NEP_TROLL_zones
...
NEPmgmtzone_centroid<-NEP_TROLL_zones %>% st_centroid() %>% st_coordinates() %>% as.data.frame()
NEPmgmtzone_centroid<-cbind(NEP_TROLL_zones$ET_ID,NEPmgmtzone_centroid)
...
ggplot() +
  ...
  coord_sf(xlim = c(-132, -25), ylim = c(0, 53.5), expand = TRUE)+
